...
input_FatXP=st.number_input("Faturamento do Mês",format="%.0f")
fxp="{:,.0f}".format(input_FatXP) 
fxp = fxp.replace(",",".")
st.caption(f"Receita Selecionada: R$ {fxp}")
input_Incremento=st.number_input("Incremento do Mês",format="%.0f")
...
nps = st.number_input("NPS Aniversário",format="%.0f")
